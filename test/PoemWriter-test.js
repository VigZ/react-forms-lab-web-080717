...
describe('<PoemWriter />', () => {
  ...
  describe('Saving input value in state', () => {
    it('should save the poem content in the state when the textarea value changes', () => {
      ...
      expect(isValueInState(wrapper.state(), VALID_POEM), 'The textarea value is not being saved in the state').to.be.true;
    });
  });
  ...
  describe('Validating the poem structure', () => {
    it('should show an error when the poem is empty', () => {
      ...
      expect(wrapper.find('#poem-validation-error').length).to.equal(1, 'The poem validation error is not being shown');
    });
    ...
    it('should not show an error when the poem is valid', () => {
      const wrapper = shallow(<PoemWriter />);
      wrapper.find('textarea').simulate('change', { target: { value: VALID_POEM } });
      console.log(wrapper.find('#poem-validation-error'));
